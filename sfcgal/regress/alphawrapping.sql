-- 3CG_3DAlphaWrapping_5
WITH cgal_version as (
  SELECT
    SPLIT_PART(
      SUBSTRING(
        postgis_sfcgal_full_version()
        FROM
          position(' CGAL ' in postgis_sfcgal_full_version()) + 5
      ),
      '.',
      1
    )::int as ver
...
SELECT 'CG_3DAlphaWrapping_5', -- cnt_pts,
  CASE
    WHEN v.ver = 5 THEN cnt_pts = 512
    ELSE cnt_pts = 520
  END as result
FROM
  (SELECT COUNT(*) as cnt_pts
   FROM ptcloud,
        LATERAL ST_DumpPoints(CG_3DAlphaWrapping(ptcloud.geom, 5)) as dumped_points) as counts,
  cgal_version v;

-- 3CG_3DAlphaWrapping_10
WITH cgal_version as (
  SELECT
    SPLIT_PART(
      SUBSTRING(
...
SELECT 'CG_3DAlphaWrapping_10', -- cnt_pts,
  CASE
    WHEN v.ver = 5 THEN cnt_pts = 584
    ELSE cnt_pts = 672
  END as result
FROM
  (SELECT COUNT(*) as cnt_pts
   FROM ptcloud,
        LATERAL ST_DumpPoints(CG_3DAlphaWrapping(ptcloud.geom, 10)) as dumped_points) as counts,
  cgal_version v;

-- 3CG_3DAlphaWrapping_15
WITH cgal_version as (
  SELECT
    SPLIT_PART(
      SUBSTRING(
...
SELECT 'CG_3DAlphaWrapping_15', -- cnt_pts,
  CASE
    WHEN v.ver = 5 THEN cnt_pts = 1080
    ELSE cnt_pts = 992
  END as result
FROM
  (SELECT COUNT(*) as cnt_pts
   FROM ptcloud,
        LATERAL ST_DumpPoints(CG_3DAlphaWrapping(ptcloud.geom, 15)) as dumped_points) as counts,
  cgal_version v;
